...
function loadLabeledImages3(){
    ...
    return new faceapi.LabeledFaceDescriptors('Test', descriptions)
}
...
async function save_data(){
    ...
    const img           = await faceapi.fetchImage(image_data_url);
    const data_wajah    = []
    const detections    = await faceapi.detectSingleFace(img).withFaceLandmarks().withFaceDescriptor()
    data_wajah.push(detections.descriptor)

    // data url of the image
    var cek_data    = Float32Array.from(data_wajah);
    var mapping     = data_wajah[0].toString();
    var nama        = $('#nama').val();
